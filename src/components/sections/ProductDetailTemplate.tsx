...
export default function ProductDetailTemplate({
  ...
  const p = product;
  const image = p.hero.image ?? "/images/pda-robot.webp";
      ...
      <SectionWrapper>
        <div className="flex flex-col items-center gap-12 lg:flex-row lg:items-center lg:gap-16">
          <div className="flex-1 text-center lg:text-left">
            {p.hero.badge && <Badge>{p.hero.badge}</Badge>}
        ...
        <div className="mt-12 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {p.benefits.items.map((b, i) => (
            <div key={i} className="rounded-xl border border-light-teal bg-white p-6 transition-shadow hover:shadow-lg">
              <span className="flex h-12 w-12 items-center justify-center rounded-xl bg-light-teal text-primary-teal">{benefitIcons[i % benefitIcons.length]}</span>
