...
function CompactList({ items }: { items: readonly string[] }) {
  return (
    <ul className="mt-4 space-y-2.5">
      {items.map((item) => (
        <li key={item} className="flex items-start gap-2.5 text-xs leading-5 text-[#64747a]">
    ...
    <div className="overflow-hidden bg-[#f3f7f6] pb-24 md:hidden">
      <section className="relative overflow-hidden bg-[#063f3c] px-5 pb-7 pt-5 text-white">
        ...
        <div className="relative">
          ...
          <p className="mt-3 max-w-[21rem] text-[1.08rem] font-semibold leading-6 text-[#8ce4dc]">{p.hero.title}</p>
      ...
      <section className="bg-white px-5 py-12">
        ...
        <div className="mt-6 flex snap-x snap-mandatory gap-3 overflow-x-auto pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {[p.concrete_benefits.patient, p.concrete_benefits.pharmacy].map((audience, index) => (
            <article key={audience.title} className="w-[82vw] max-w-[20rem] shrink-0 snap-center rounded-[1.5rem] bg-[#f3f7f6] p-5">
              <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#063f3c] text-sm font-black text-white">0{index + 1}</span>
              <h3 className="mt-4 font-sans text-xl font-bold text-[#15242d]">{audience.title}</h3>
              <p className="mt-2 text-xs leading-5 text-[#64747a]">{audience.description}</p>
      ...
      <section className="px-5 py-12">
        ...
        <div className="mt-6 overflow-hidden rounded-[1.5rem] border border-[#dce9e7]">
          <div className="bg-white p-5">
            <p className="text-[10px] font-extrabold uppercase tracking-wider text-[#89969a]">{p.comparison.manual_title}</p>
            <CompactList items={p.comparison.manual_items} />
          </div>
          <div className="bg-[#063f3c] p-5 text-white">
            <p className="text-[10px] font-extrabold uppercase tracking-wider text-[#81ded6]">{p.comparison.dual_title}</p>
            <ul className="mt-4 space-y-2.5">
              {p.comparison.dual_items.map((item) => (
                <li key={item} className="flex items-start gap-2.5 text-xs leading-5 text-white/75">
                  <span className="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-[#f5c518] text-[#102c2b]">✓</span>
                  {item}
                </li>
              ))}
            </ul>
      ...
      <section className="bg-white px-5 py-12">
        ...
        <div className="mt-6 overflow-hidden rounded-[1.4rem] bg-[#102c2b] p-1.5 shadow-lg">
        ...
        <ol className="mt-7 space-y-3">
          {p.workflow.steps.map((step, index) => (
            <li key={step.title} className="flex gap-3 rounded-2xl bg-[#f3f7f6] p-4">
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#007f78] text-xs font-black text-white">{String(index + 1).padStart(2, "0")}</span>
              <div>
                <h3 className="font-sans text-sm font-bold text-[#15242d]">{step.title}</h3>
                <p className="mt-1 text-[11px] leading-4 text-[#64747a]">{step.description}</p>
              </div>
            </li>
          ))}
        </ol>
      ...
      <section className="px-5 py-12">
        ...
        <Link href={contactHref(locale, "quote")} className="mt-4 flex min-h-12 items-center justify-between rounded-xl border border-[#cfe2df] bg-white px-4 text-xs font-extrabold text-[#007f78]">
          {p.support.cta_quote}
          <MobileArrow />
        </Link>
      </section>
