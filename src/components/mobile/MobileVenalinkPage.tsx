  ...
  return (
    <div className="overflow-hidden bg-[#f3f7f6] pb-24 md:hidden">
      ...
      <section className="bg-white px-5 py-12">
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          {p.solution.formats.map((row) => (
            <article key={row.format} className="rounded-2xl border border-[#dce9e7] bg-[#f3f7f6] p-4">
              <h3 className="font-sans text-lg font-black text-[#15242d]">{row.format}</h3>
              <p className="mt-2 text-[11px] leading-4 text-[#64747a]">{row.usage}</p>
            </article>
          ))}
        </div>
        <div className="mt-7">
          <h3 className="font-sans text-sm font-bold text-[#15242d]">{p.solution.patients_title}</h3>
      ...
      <section className="px-5 py-12">
        ...
        <p className="mt-3 text-sm leading-6 text-[#64747a]">{p.accessories.description}</p>
        <div className="mt-6 space-y-2.5">
          {p.accessories.rows.map((row, index) => (
            <article key={row.accessory} className="flex gap-3 rounded-2xl border border-[#dce9e7] bg-white p-4">
              <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#e0f5f3] text-[10px] font-black text-[#007f78]">
                {String(index + 1).padStart(2, "0")}
              </span>
              <div>
                <h3 className="font-sans text-sm font-bold text-[#15242d]">{row.accessory}</h3>
                <p className="mt-1 text-[11px] leading-4 text-[#64747a]">{row.role}</p>
              </div>
            </article>
          ))}
        </div>
      ...
      <section className="bg-[#007f78] px-5 py-12 text-center text-white">
        <p className="font-bold uppercase tracking-wider text-[#f5c518]">{p.starter.eyebrow}</p>
